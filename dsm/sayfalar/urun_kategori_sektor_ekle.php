...
$urun_kategori_id = $_GET['urun_kategori_id'];

$urun_bilgisi = Sonuc(Sorgu("SELECT * FROM urun_kategori WHERE id = $urun_kategori_id"));

$hizmet_keys=Sorgu("SELECT * FROM urun_kategori_sektor WHERE urun_kategori_id='$urun_kategori_id'");
$hizmet_sonuc = "";
while($veri = mysqli_fetch_object($hizmet_keys)) { 
	$hizmet_sonuc .= $veri->sektor_id .","; 
}
...
if($_POST['Kaydet']=="Kaydet")
{
	...
	Sorgu("DELETE FROM urun_kategori_sektor WHERE urun_kategori_id='$urun_kategori_id'");
	if ($keys55 != null) {
		$explode55 = explode(",", $keys55);
		foreach($explode55 as $element55)
		{
			$hizmet_ekle_sorgu = Sorgu("INSERT INTO urun_kategori_sektor (urun_kategori_id, sektor_id) VALUES (".$urun_kategori_id.",".$element55.")");
		}
		if($hizmet_ekle_sorgu){
			echo '<script>alert("Katgeoriye Sektör Eklendi")</script>';
			header("Location: $sayfa_url");	
		}else{
			echo '<script>alert("Katgeoriye Sektör Eklenemedi")</script>';
		}
	}
...
{
	$gid = $_GET['gid'];
	$hizmet_sil_sorgu = Sorgu("DELETE FROM urun_kategori_sektor WHERE id='$gid'");
}
...
<head>
	...
	<title><?=$urun_bilgisi->page_name?> Kategorisine Sektör Ekleme</title>
	<!-- Bootstrap 3.3.2 -->
...
<body>
	...
	<div class="col-md-12">
		...
		<div class="row">
			<div class="col-md-3">
				<h4>Sektör Ekle / Sil</h4>
				<!-- DIRECT CHAT PRIMARY -->
				<div class="box box-success direct-chat direct-chat-success">
					<div class="box-header with-border">
						<h3 class="box-title" style="margin: 25px 0 0 0;text-align: center;"><?=$urun_bilgisi->page_name?> - Kategorisine Bağlı Sektörler </h3>
					</div><!-- /.box-header --> 
					<div class="box-body">
						<div class="col-md-12" style="text-align: center;margin:10px 0;"> 
							<input type="text" name="marka_ara_text" id="marka_ara_text" placeholder="Sektör Ara :" style="padding: 5px; width: 100%" />
							<?php 
							$gelendeger = "";
						...
						<form action="#" method="post" enctype="multipart/form-data">
							...
							<div class="direct-chat-messages row">
								...
								<div class="direct-chat-msg col-md-12">
									...
									<?php $marka_ara = Sorgu("SELECT id,page_name FROM sektor WHERE statu = 1 " . $gelendeger);
									$num = 0;
