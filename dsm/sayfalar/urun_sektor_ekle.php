...
$hizmet_keys=Sorgu("SELECT * FROM urun_sektor WHERE urun_id='$urun_id'");
$hizmet_sonuc = "";
$kullanim_arr = [];
while($veri = mysqli_fetch_object($hizmet_keys)) { 
	$hizmet_sonuc .= $veri->sektor_id .","; 
	$kullanim_arr[] = $veri->sektor_id;
}
...
if($_POST['Kaydet']=="Kaydet")
{
	...
	Sorgu("DELETE FROM urun_sektor WHERE urun_id='$urun_id'");
	if ($keys55 != null) {
		$explode55 = explode(",", $keys55);
		foreach($explode55 as $element55)
		{
			$hizmet_ekle_sorgu = Sorgu("INSERT INTO urun_sektor (urun_id, sektor_id) VALUES (".$urun_id.",".$element55.")");
		}
		if($hizmet_ekle_sorgu){
			echo '<script>alert("Ürüne Sektör Eklendi")</script>';
			header("Location: $sayfa_url");	
		}else{
			echo '<script>alert("Ürüne Sektör Eklenemedi")</script>';
		}
	}
...
{
	$gid = $_GET['gid'];
	$hizmet_sil_sorgu = Sorgu("DELETE FROM urun_sektor WHERE id='$gid'");
}
...
<head>
	...
	<title><?=$urun_bilgisi->page_name?> Ürününe Sektör Ekleme</title>
	<!-- Bootstrap 3.3.2 -->
			...
			<div class="col-md-3">
				...
				<div class="box box-success direct-chat direct-chat-success">
					<div class="box-header with-border">
						<h3 class="box-title" style="margin: 25px 0 0 0;text-align: center;"><?=$urun_bilgisi->page_name?> - Ürününe Bağlı Sektörler </h3>
					</div><!-- /.box-header --> 
					<div class="box-body">
										...
										<div class="col-lg-12">
											<div class="input-group">
												<span class="input-group-addon" id="checkboxes">  
													<input name="secenek_kategori1[]" type="checkbox" class="markacls" id="marka<?=$num?>" value="<?php echo $SecenekSonuc1->id;?>" <?php if(in_array($SecenekSonuc1->id, $kullanim_arr)){echo "checked";}?>>
													<?php $num++; ?>
												</span>
