...
if(p('title') && g('islem')=="")
{
  ...
  if ($upload->uploaded){
    ...
    $upload->process("../uploads/catalog");
    if ($upload->processed){
      $KatalogResim=''.$upload->file_dst_name.'';
    }
  }
  $gitti=$KatalogResim=''.$upload->file_dst_name.'';

  $upload2 = new upload($_FILES['image_webp']);
  if ($upload2->uploaded){
    $upload->file_auto_rename = true;
    $upload->file_new_name_body = seo($title).'-'.seo($ayar['firma_adi']);
    $upload2->process("../uploads/catalog/other");
  ...
  $sayfa_sorgu = Sorgu("INSERT INTO kataloglar SET
...
if(p('title') && g('islem')=="duzenle")
{
  ...
  $update_sql = "";
  $resim_bul=Sonuc(Sorgu("SELECT * FROM kataloglar WHERE id='$d_id'"));
  if($KatalogResim!="")
  {
      if(file_exists("../uploads/catalog/".$resim_bul->image)){
          $resim_sil=unlink("../uploads/catalog/".$resim_bul->image);
      }
      $gitti=$KatalogResim=''.$upload->file_dst_name.'';
      $update_sql .= "image = '$KatalogResim', ";
  }
  
  if($image_webp!="")
  {
      if(file_exists("../uploads/catalog/other/".$resim_bul->image_webp)){
          $resim_sil=unlink("../uploads/catalog/other/".$resim_bul->image_webp);
      }
      $gitti2=$image_webp=''.$upload2->file_dst_name.'';
      $update_sql .= " image_webp = '$image_webp', ";
  }
  
  $Katalog_duzenle_sorgu = Sorgu("UPDATE kataloglar SET 
      title = '$title', title2 = '$title2',
    en_title = '$en_title', en_title2 = '$en_title2',
    de_title = '$de_title', de_title2 = '$de_title2',
    ru_title = '$ru_title', ru_title2 = '$ru_title2',
...
if($_GET['islem']=="duzenle")
{
  $sayfaid = $_GET['id']; 
  $durum = "duzenle" ;
  $SayfaSonuc = Sonuc(Sorgu("SELECT * FROM kataloglar WHERE id='$sayfaid'"));
}
/* Genel Sıra */
$son_sira_sorgu = Sonuc(Sorgu("SELECT * FROM kataloglar ORDER BY `rank` DESC LIMIT 1"));
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <small><i class="fa fa-picture-o"></i> Katalog Ekle</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="anasayfa.html"><i class="fa fa-home"></i> Anasayfa</a></li>
            <li class="active">Katalog Ekle</li>
        </ol>
    </section>
    <!-- Main content -->
                ...
                <div class="col-md-7">
                    <div class="nav-tabs-custom">
                        <ul class="nav nav-tabs">
                          ...
                          <?php $Katalog_LangSorgu = Sorgu("SELECT * FROM dil WHERE statu = 1 ORDER BY id ASC"); while($Katalog_LangSonuc = mysqli_fetch_array($Katalog_LangSorgu)){?>
                          <li <?php if ($Katalog_LangSonuc['id']==1): ?>style="display: none;"<?php endif ?>><a href="#<?=$Katalog_LangSonuc['id']?>" data-toggle="tab"><?=$Katalog_LangSonuc['name']?></a></li>
                          <?php } ?>
                        </ul>
                        <div class="tab-content">
                                ...
                                <div class="box box-primary">
                                    ...
                                    <div class="box-body">
                                        <div class="form-group">
                                            <label for="title">Katalog Adı </label>
                                            <input type="text" class="form-control" name="title" value="<?php echo $SayfaSonuc->title;?>" id="title" placeholder="Katalog Adı" required="true" tabindex="1" autofocus="" />
                                        </div>
                                    </div>
                            ...
                            <?php $Katalog_LangSorgu2 = Sorgu("SELECT * FROM dil WHERE statu = 1 ORDER BY id ASC"); while($Katalog_LangSonuc2 = mysqli_fetch_array($Katalog_LangSorgu2)){$code=$Katalog_LangSonuc2['code'];?>
                            <div class="tab-pane" id="<?=$Katalog_LangSonuc2['id']?>">
                                <div class="box box-primary">
                                    <div class="box-header"><br><h3 class="box-title"><?=$Katalog_LangSonuc2['name']?> İçerikler</h3><hr style="margin-top: 15px;margin-bottom: 0px;"></div>
                                    <div class="box-body">
                                        <div class="form-group">
                                            <label for="<?=$Katalog_LangSonuc2['code']?>_title">Katalog Adı </label>
                                            <?php if ($code == "en"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->en_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                            <?php if ($code == "de"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->de_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                            <?php if ($code == "ru"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->ru_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                            <?php if ($code == "fr"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->fr_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                            <?php if ($code == "es"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->es_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                            <?php if ($code == "it"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->it_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                            <?php if ($code == "ar"): ?><input type="text" class="form-control" name="<?=$Katalog_LangSonuc2['code']?>_title" value="<?php echo $SayfaSonuc->ar_title;?>" id="<?=$Katalog_LangSonuc2['code']?>_title" placeholder="Katalog Adı" required="true" /><?php endif ?>
                                        </div>
                                    </div>
                    ...
                    <div class="box box-danger box-solid">
                        ...
                        <div class="box-body">
                            <div class="row">
                                <div class="form-group">
                                    <div class="col-md-6">                                      
                                        <label>Katalog Kapak Resmi</label>
                                        <div class="fileupload fileupload-new" data-provides="fileupload">
                                            <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
                                                <?php if($_GET['islem']=='duzenle'){?>
                                                <?php if($SayfaSonuc->image){?>
                                                <a href="../uploads/catalog/<?php echo $SayfaSonuc->image;?>" target="_blank"><img src="../uploads/catalog/<?php echo $SayfaSonuc->image;?>" alt="" /></a>
                                                <?php } else { ?>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="fileupload fileupload-new" data-provides="fileupload">
                                            <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
                                                ...
                                                <a href="../uploads/catalog/other/<?php echo $SayfaSonuc->image_webp;?>" target="_blank"><img src="images/pdf_file.png" alt="" /></a>
                                                <?php } else { ?>
                                            ...
                                            <div>
                                                <span class="btn btn-default btn-file">
                                                    <span class="fileupload-new"><i class="fa fa-paper-clip"></i> Katalog Seç</span>
                                                    <span class="fileupload-exists"><i class="fa fa-undo"></i> Değiştir</span>
